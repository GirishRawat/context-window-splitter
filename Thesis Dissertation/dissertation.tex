...
\section{Methodology: The Fail-Safe Pipeline Architecture}

The core contribution of this project is a robust, six-phase orchestration pipeline. It intercepts the standard build system, manages the lifecycle of each function as it is parsed and optimized, and rigorously verifies the output before relinking. Figure \ref{fig:arch} illustrates the complete pipeline architecture.
...
\begin{tikzpicture}[
    ...
    box/.style={rectangle, draw=black, rounded corners, minimum height=0.8cm, text centered, font=\sffamily},
    arrow/.style={thick,->,>=stealth}
]

\node (start) [box, fill=blue!10] {1. IR Parsing \& Sanitization};
\node (triage) [box, fill=blue!10, below=of start] {2. Triage \& Complexity Analysis};
\node (routing) [box, fill=blue!10, below=of triage] {3. Model Routing};
\node (inference) [box, fill=orange!20, below=of routing] {4. LLM Inference (Zero-Shot)};
\node (reconstruct) [box, fill=blue!10, right=of inference] {5. Candidate Reconstruction};
\node (verify) [box, fill=red!20, above=of reconstruct] {6. Formal Verification (Alive2)};
\node (fallback) [box, fill=green!20, right=of verify] {Fallback to Original IR};
\node (success) [box, fill=green!20, above=of verify] {Adopt Optimized IR};
\node (compile) [box, fill=gray!20, above=of success] {7. Module Assembly \& Clang};

\draw [arrow] (start) -- (triage);
\draw [arrow] (triage) -- (routing);
\draw [arrow] (routing) -- (inference);
\draw [arrow] (inference) -- (reconstruct);
\draw [arrow] (reconstruct) -- (verify);
\draw [arrow] (verify) -- node[anchor=south] {FAIL} (fallback);
\draw [arrow] (verify) -- node[anchor=east] {PASS} (success);
\draw [arrow] (fallback) |- (compile);
\draw [arrow] (success) -- (compile);

\end{tikzpicture}
}
\caption{The Deterministic Fail-Safe Orchestration Pipeline}
\label{fig:arch}
\end{figure}
...
Phase 2 calculates the cyclomatic complexity of each extracted function by analyzing its control flow graph (CFG). Functions falling below a configurable threshold are marked as triaged and skip the optimization phases entirely. Table \ref{tab:triage} defines the strict routing and triage boundaries.

\begin{table}[htbp]
\caption{Triage Configuration \& Model Routing Tiers}
\label{tab:triage}
\begin{center}
\begin{tabular}{|l|l|l|}
\hline
\textbf{Tier Name} & \textbf{Token Limit} & \textbf{Assigned Model (Local)} \\
\hline
Triaged & N/A & Skipped (Complexity $<$ 5) \\
Fast & 0 - 8,000 & Qwen 2.5 Coder 3B \\
Mid & 8,000 - 32,000 & Qwen 2.5 Coder 7B \\
Frontier & $>$ 32,000 & Claude 3 Opus (API) \\
\hline
\end{tabular}
\end{center}
\end{table}
...
Functions that pass the triage phase are routed to an available AI model based on their token count. The system supports local inference using Ollama, which allows for zero-cost, privacy-preserving experimentation. 
...
To rigorously evaluate the pipeline, experiments were conducted using the official \texttt{llvm-test-suite} and the \texttt{sbase} Unix utilities corpus. The test suite contains a wide variety of computational workloads, including fast Fourier transforms (FFT), Mandelbrot set generation, matrix multiplication, and floating-point benchmarks (e.g., \texttt{fbench}, \texttt{flops}). 
...
\begin{table}[htbp]
\caption{Corpus Dataset Breakdown}
\label{tab:datasets}
\begin{center}
\begin{tabular}{|l|c|c|c|}
\hline
\textbf{Dataset} & \textbf{Total Functions} & \textbf{Functions Processed} & \textbf{Mean Tokens} \\
\hline
\texttt{sbase} & 645 & 311 & 4,120 \\
\texttt{llvm-test-suite} & 124 & 100 & 8,534 \\
\hline
\end{tabular}
\end{center}
\end{table}
...
The evaluation on the real-world benchmark suite revealed significant challenges regarding local model capabilities, but proved the absolute robustness of the orchestration pipeline. Figure \ref{fig:complexity} visualizes the distribution of function complexity versus context window size across the two datasets.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.9\linewidth]{complexity_plot.pdf}
\caption{Scatter plot of function cyclomatic complexity versus token count across the \texttt{sbase} and \texttt{llvm-test-suite} datasets. Most functions fall comfortably within the local LLMs' context windows.}
...
\subsection{Case Study: Model Hallucination}
To understand why the models failed so frequently, we manually inspected the inference output for several benchmark functions. The listing below shows an abridged, typical response from the \texttt{qwen2.5-coder:3b} model when tasked with optimizing the main function of the \texttt{tar} utility.

\begin{lstlisting}[caption={Example of local LLM conversational hallucination}, label={lst:hallucination}]
Sure! The provided LLVM IR code contains a lot of redundant basic blocks and memory allocations that can be combined. Here is the optimized version:

```llvm
define dso_local i32 @main(i32 noundef %0, ptr noundef %1) {
  ; I have simplified the control flow here
  ; and removed the dead alloca instructions.
  ...
}
```

This code should now run much faster because we skipped the unnecessary allocations. Let me know if you need any more help!
\end{lstlisting}

While the model correctly wrapped the response in a markdown block, the output completely lacked the mandatory function preamble, missed crucial variable type definitions, and introduced arbitrary comments. The \texttt{llvm-as} tool instantly flagged this output as invalid syntax.
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.9\linewidth]{verdict_plot.pdf}
\caption{Pipeline Verdict Distribution. The graph illustrates that 100\% of the LLM responses on the \texttt{sbase} and \texttt{llvm-test-suite} datasets resulted in syntax failures or Alive2 timeouts, triggering a perfectly safe fallback.}
\label{fig:verdicts}
\end{figure}

Every single malformed response, syntax error, and hallucination generated by the AI was caught during Phase 4 (Candidate Reconstruction) or Phase 5 (Formal Verification). As shown in Figure \ref{fig:verdicts}, the system recorded a 100\% catch rate for invalid optimizations. The functions were correctly flagged with ``SYNTAX\_FAIL'' or ``PENDING'' verdicts. Table \ref{tab:verdicts} breaks down the exact numerical outcomes for the \texttt{llvm-test-suite}.

\begin{table}[htbp]
\caption{Verdict Breakdown: llvm-test-suite}
\label{tab:verdicts}
\begin{center}
\begin{tabular}{|l|c|}
\hline
...
\hline
Triaged (Skipped, Complexity $<$ 5) & 24 \\
Syntax Fails (Caught by llvm-as) & 68 \\
Alive2 Timeouts (Triggered Fallback) & 32 \\
Rejected by Alive2 (Caught Error) & 0 \\
Passed (Successfully Optimized) & 0 \\
\hline
\end{tabular}
\end{center}
\end{table}

Phase 6 executed its fallback logic perfectly, reverting all failing functions to their original, unoptimized states. The orchestrator then compiled the resulting IR into object files, completely masking the AI's failure from the build system.
